...
#
# Commands:
#   hubot bittrex <ticker> - Get latest BTC ticker data from Bittrex
#   hubot poloniex <ticker> - Get latest BTC ticker data from Poloniex
#   hubot cryptodao <ticker> - Get latest BTC ticker data from CryptoDAO
#   hubot coinexchange <ticker> - Get latest BTC ticker data from CoinExchange
#   hubot coinsmarkets <ticker> - Get latest BTC ticker data from CoinsMarkets
#   hubot nova <ticker> - Get latest BTC ticker data from Novaexchange
#
# Author:
#   upgradeadvice
#
...
module.exports = (robot) ->
  ...
  #coinexch
  robot.respond /coinexchange (.*)/i, (msg) ->
    c = msg.match[1].toUpperCase()
    cU = c.toUpperCase()
    u = "https://www.coinexchange.io/api/v1/getmarkets"
    msg.robot.http(u).headers(h).get() (err, res, body) ->
      try
        d = JSON.parse(body)
        if not d.success
          msg.reply "#{cU} market not found"
          return
      catch err
        msg.reply "Error looking up #{cU}"
        return
      d = JSON.parse(body)
      code = (d.result.where "MarketAssetCode":c, "BaseCurrencyCode":"BTC")[0]["MarketID"]
      console.log("Code: " + code)
      u = "https://www.coinexchange.io/api/v1/getmarketsummary?market_id=#{code}"
      msg.robot.http(u).headers(h).get() (err, res, body) ->
        try
          d = JSON.parse(body)
          if not d.result["LastPrice"]
            msg.reply "No results for #{cU}"
            return
        catch err
          msg.reply "No results for #{cU}"
        ...
        d = JSON.parse(body)
        console.log(d)
        last = d.result["LastPrice"]
        high = d.result["HighPrice"]
        low = d.result["LowPrice"]
        vol = d.result["Volume"]
        change = d.result["Change"]
        msg.reply "[OK] #{cU}-BTC on CoinExchange.io [Last: #{last}] -
        [High: #{high}] - [Low: #{low}] - [Vol: #{vol}] - [24h Change: #{change}%]"

  #coinexch
  robot.respond /coinsmarkets (.*)/i, (msg) ->
    c = msg.match[1].toUpperCase()
    market = "BTC_" + c
    cU = c.toUpperCase()
    u = "https://coinsmarkets.com/apicoin.php"
    msg.robot.http(u).headers(h).get() (err, res, body) ->
      try
        d = JSON.parse(body)
        if not d[market]
      ...
      last = d[market]["last"]
      high = d[market]["high24hr"]
      low = d[market]["low24hr"]
      vol = d[market]["24htrade"]
      bid = d[market]["highestBid"]
      ask = d[market]["lowestAsk"]
      msg.reply "[OK] #{cU}-BTC on CoinsMarkets.com [Last: #{last}] -
      [High (24hr): #{high}] - [Low (24hr): #{low}] - [Bid: #{bid}] - [Ask: #{ask}] - [Trades (24hr): #{vol}]"


  #cryptodao
  robot.respond /cryptodao (.*)/i, (msg) ->
    c = msg.match[1].toUpperCase()
    cU = c.toUpperCase()
    u = "https://cryptodao.com/api/ticker?source=BTC&target=#{c}"
    msg.robot.http(u).headers(h).get() (err, res, body) ->
      try
        d = JSON.parse(body)
        if not d.sell
      ...
      vol = d.vol
      buy = d.buy.toString()
      sell = d.sell.toString()
      msg.reply "[OK] #{cU}-BTC on CryptoDAO [Last: #{last}] -
      [High: #{high}] - [Low: #{low}] - [Vol: #{vol}] - [Buy: #{buy}] - [Sell: #{sell}]"

  #novaexchange
  robot.respond /nova (.*)/i, (msg) ->
    c = msg.match[1].toUpperCase()
    cU = c.toUpperCase()
    u = "https://novaexchange.com/remote/v2/market/info/BTC_#{c}/"
    msg.robot.http(u).headers(h).get() (err, res, body) ->
      try
        d = JSON.parse(body)
        if not d.markets
      ...
      buy = d["bid"]
      sell = d["ask"]
      msg.reply "[OK] #{cU}-BTC on NovaExchange [Last: #{last}] -
      [High: #{high}] - [Low: #{low}] - [Vol: #{vol}] - [Buy: #{buy}] - [Sell: #{sell}]"
